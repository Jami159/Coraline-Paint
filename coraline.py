...
#stolen and modified from sir's getname function
...
def filltool(mx, my):
    ...
    if pixCol==colour: 
        return
    edge=[(mx-250,my-150)]          #appends to list the spot you clicked on
    canvas.set_at((mx-250, my-150), (colour))   #changes colour
    for (mx, my) in edge:       #goes through points around where clicked
        for (x, y) in ((mx+1, my), (mx-1, my), (mx, my+1), (mx, my-1)):
            if 0<mx<799 and 0<my<599:
                if canvas.get_at((x, y))==pixCol:       #if the new points'
                                                        #colour matches point
                                                        #where clicked, it
                    canvas.set_at((x, y), (colour))     #changes colour   
                    edge.append((x, y))                 #it adds those points to the
                                                        #list, keeps adding and
                                                        #filling until enclosed
                                                        #shape is filled in
    screen.blit(canvas,(250,150))


#clears canvas using same concept as eraser tool
def cleartool():    
...
def polygontool():
    ...
    if mb[0] == 1:
        screen.set_clip(canvasRect)
        screen.blit(newscreen,(0,0))
        if firstpoint == False:     #starts the new shape
            draw.line(screen,colour,(startx,starty),(mx,my),thick)
        elif firstpoint == True:    #draws from the end point of the last line
                                    #to mouse position
...
#cuts and moves selected area/ can also copy
def selecttool():
...
display.set_caption("Coraline Paint")
...
#stolen and modified from yining
...
#blits stamps along with transformations
def stamps_blit():
    screen.set_clip((canvasRect))    
    for i in range(len(stampList)):
        if tool==stampList[i]:
...
while running:
    ...
    for evnt in event.get():
        ...
        if evnt.type==MOUSEBUTTONDOWN:
            ...
            if canvasRect.collidepoint(mx,my) and evnt.button==1 and tool!="" and tool!="eyedropper":
               undolist.append(canvas.copy())  #adds a picture of screen to undolist
                                               #after mouse is pressed down in the canvas 
               first=True
               redolist=[]
            elif clearRect.collidepoint(mx,my) and evnt.button==1:
                undolist.append(canvas.copy())
            if undorect.collidepoint(mx,my) and evnt.button==1:
                if first:
                    redolist.append(canvas.copy())
                    first=False         #adds a copy of the canvas to redolist
                                        #after udno button is pressed
                if len(undolist)>1:
                    cleartool()                    
                    canvas.blit(undolist[-1],(0,0)) #blits the last thing in the undolist
                    redolist.append(undolist[-1])  #adds to redolist
                    undolist.pop()   #removes the last thing in undolist
                    
            elif redorect.collidepoint(mx,my) and evnt.button==1:
                if len(redolist)>1:
                    cleartool()
                    canvas.blit(redolist[-2],(0,0)) #blits the second last thing in the redolist
                                                    #because last item is blank canvas
                    undolist.append(redolist[-1])   #adds last item inredo list to undolist
                    redolist.pop()                  #deletes last item in redolist
        ...
        if evnt.type==MOUSEBUTTONUP:
            if undorect.collidepoint(mx,my) and evnt.button==1: #for tools that need blitting
                                                                #since they blit after MBU
                                                                #undo button blits background after
                                                                #MBU to make most recent drawing disappear
                screen.blit(background.subsurface(canvasRect),(250,150))
                screen.blit(canvas,(250,150)) 


            #for tools that require blitting on a newscreen, since it's hard to
            #move the picture around on a see through canvas, these tools are only
            #blit after the mouse is released
    ...
    if saverect.collidepoint(mx,my) and mb[0]==1:
        save=True
    if save==True:  #gives the user the option of if they want to save or not
                    #incase they clicked the save button on accident
    ...
    if loadrect.collidepoint(mx,my) and mb[0]==1:
        load=True
    if load==True:  #gives the user the option of if they want to load or not
                    #incase they clicked the load button on accident
        screen.blit(image.load("Image/SaveLoad.png"),(27,525))
        if yesrect.collidepoint(mx,my) and mb[0]==1:
            yes=True
        if yes==True:

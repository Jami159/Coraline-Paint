...
def texttool():
    ...
    if canvasRect.collidepoint((mx,my)):
        ...
        if keys.index(1)<300 and slowdown:
            if keys[8]==1:
                canvas.blit(screen,(0,0))
                ans = ans[0:len(ans)-1]
            else:
                ans += chr(keys.index(1))
        slowdown=False
        if keys.index(1)==300 :
            slowdown = True
        txtPic = comicFont2.render(ans, True, (colour))
        canvas.blit(txtPic,(startx-152,starty-100))  
...
saverect=draw.rect(screen,(255,255,255),(950,500,25,25))

loadrect=draw.rect(screen,(255,255,255),(950,550,25,25))
...
first=True
while running:
    comicFont2 = font.SysFont("Comic Sans MS", thick)    
    for i in range(len(stampList)):
        if tool==stampList[i]:
            stamp_image=image.load("images/N"+stampList[i]+".png")
            changedstamp = transform.rotozoom(stamp_image, turn, size)
    ...
    for evnt in event.get():
        ...
        if evnt.type==MOUSEBUTTONUP:
            if undorect.collidepoint(mx,my) and evnt.button==1:
                if canvas_erase=="arena":
                    screen.blit(arena.subsurface(0,0,720,480),(152,100))
                elif canvas_erase=="tablet":
                    screen.blit(tablet.subsurface(0,0,720,480),(152,100))
                elif canvas_erase=="duel":
                    screen.blit(duel.subsurface(0,0,720,480),(152,100))
                else:
                    screen.blit(background.subsurface(canvasRect),(152,100))
            ...
            if evnt.button==1 and canvasRect.collidepoint(startx,starty):
                if tool=="line":
                    ...
                    if canvasRect.collidepoint((startx,starty)):                        
                        if firstpoint == False:
                            ...
                            draw.line(canvas,colour,(nx-152,ny-100),(mx-152,my-100),thick)
                        nx,ny = mx,my
                        closed = False
                        firstpoint = True
                if canvasRect.collidepoint(mx,my):
                ...
                if tool=="select":
                    ...
                    if closed == False:
                        draw.line(canvas,colour,(a-152,b-100),(nx-152,ny-100),thick)
                    closed = True
                    firstpoint = False 
    ...
    if saverect.collidepoint(mx,my) and mb[0]==1:# tool=="Save":
        ...
        if "." not in txt:
            txt+=".jpg"
        screen7=canvas.copy()                   # I lost count 
        image.save(screen7.subsurface(0,0,720,480),(txt))# Saves all the images in a seprate folder
        tool="pencil"
